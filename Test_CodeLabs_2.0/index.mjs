...
export const handler = async (event) => {
  ...
  const headers = {
    "Access-Control-Allow-Origin":
      "http://vccodelabs.s3-website-us-east-1.amazonaws.com",
  ...
  return new Promise((resolve, reject) => {
    ...
    busboy.on("finish", async () => {
      ...
      try {
        ...
        console.info("Fetched repository:", repository);
        ...
        console.info("Fetched ref:", ref);

        const postRef = await githubRequest(
          "POST",
          `/repos/${encodeURIComponent(repoOwner)}/${encodeURIComponent(
            repoName
          )}/git/refs`,
        ...
        console.info("Posted ref:", postRef);
        ...
        for (const file of files) {
          ...
          try {
            const existingFile = await githubRequest(
              ...
              `/repos/${encodeURIComponent(repoOwner)}/${encodeURIComponent(
                repoName
              )}/contents/${encodeURIComponent(filePath)}?ref=${branchName}`
            );
            console.info("Fetched existing file:", existingFile);
          ...
          const postFile = await githubRequest(
            "PUT",
            `/repos/${encodeURIComponent(repoOwner)}/${encodeURIComponent(
              repoName
          ...
          console.info("Posted file:", postFile);
        }
        ...
        const existingPRs = await githubRequest(
          ...
          `/repos/${encodeURIComponent(repoOwner)}/${encodeURIComponent(
            repoName
          )}/pulls?head=${repoOwner}:${branchName}`
        );
        console.info("Fetched existing PRs:", existingPRs);
